...
import { Mail, Lock, User, Building2, Sparkles, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
...
export const RegisterPage = ({ onSwitchToLogin }: RegisterPageProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    registerMutation.mutate({
      ...
      // Department and Role handling would need backend support or custom data fields if not already present
    });
  };
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['student', 'faculty'].map((role) => (
                    <button
                      ...
                      disabled={registerMutation.isPending}
                      className={`px-5 py-3.5 rounded-2xl border-2 transition-all duration-300 font-medium ${formData.role === role
